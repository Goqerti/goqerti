...
def help(update, context):
    update.message.reply_text('*Oyun Haqqında* Botu grupa qat *ADMIN* et daha sonra oyuna başla\nAparıcı olan sözə baxır və digər oyunçulara başa salır sözü tapan şəxs aparıcı olur. Sözü başa sala bilmirsənsə *Sözü dəyiş* düyməsi ilə sözü dəyişə bilər\n\n', reply_to_message_id=True,
...
def button(update, context):
    ...
    if query.data == 'change_word':
        ...
        if game.is_master(query.from_user.id):
            bot.answer_callback_query(callback_query_id=query.id, text=word, show_alert=True)
# https://telegra.ph/file/a86efd2857953e09b3a47.mp4
BDT_VIDEO = "https://telegra.ph/file/b9a705694aae53c81dc8a.mp4"

def command_start(update, context: CallbackContext):
    if update.effective_chat.type == "private":
        
        addme = InlineKeyboardButton(text="🦎 Məni Qrupa əlavə et!", url="https://t.me/BDTKrasvortbot?startgroup=a")
        kanal = InlineKeyboardButton(text="📣 Lahiyə Kanalı", url="https://t.me/botfatheraz")
        grup = InlineKeyboardButton(text="🧑‍💻 Rəsmi Grup", url="https://t.me/BDTGame")
        ...
        game = get_or_create_game(chat_id)
        game.start()
        update.message.reply_text('Oyunu Başladı! Tələsin!🦎'.format(username), reply_to_message_id=True)
...
def set_master(update, context):
    ...
    update.message.reply_text('Hazırda [{}](tg://user?id={}) Aparıcıdır'.format(username,user_id), reply_to_message_id=True, reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN)
...
def command_master(update: Update, context):
    ...
    if not game.is_master_time_left():
        update.message.reply_text('Aparıcı olmaq üçün {} saniyə qaldı'.format(game.get_master_time_left()),
                                  reply_to_message_id=True)
        return
